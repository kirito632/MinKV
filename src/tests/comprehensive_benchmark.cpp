...
void save_to_csv(const std::vector<BenchmarkResult> &results,
  ...
  file << "# Optimization Version: v2.0 (100w keys, thread-local stats, 10w "
          "vectors)\n";
  file << "#\n";

  // CSV头部
  file << "Test,Threads,TotalOps,Duration(ms),QPS,AvgLatency(us),P50(us),P95("
          "us),P99(us),HitRate(%)\n";

  // 数据行
  for (const auto &r : results) {
    file << r.test_name << "," << r.thread_count << "," << r.total_ops << ","
...
void print_results(const std::vector<BenchmarkResult> &results) {
  ...
  std::cout << std::left << std::setw(20) << "测试场景" << std::right
            << std::setw(8) << "线程数" << std::setw(12) << "QPS"
            << std::setw(12) << "P50(us)" << std::setw(12) << "P95(us)"
            << std::setw(12) << "P99(us)" << std::setw(10) << "命中率" << "\n";
  std::cout << std::string(86, '-') << "\n";

  for (const auto &r : results) {
    std::cout << std::left << std::setw(20) << r.test_name << std::right
              << std::setw(8) << r.thread_count << std::setw(12) << std::fixed
...
int main() {
  ...
  for (int threads : {1, 2, 4, 8}) {
    std::cout << "  - " << threads << " 线程:\n";
    auto result =
        benchmark_vector_search(threads, 500); // 减少搜索次数，因为数据量大了
    results.push_back(result);
    std::cout << "    QPS: " << std::fixed << std::setprecision(0) << result.qps
              << ", P99: " << std::fixed << std::setprecision(2)
              << result.p99_latency_us << "μs\n";

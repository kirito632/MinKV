...
const char* wal_mode_name(WALMode mode) {
    switch (mode) {
        case WALMode::NONE: return "纯内存(WAL关闭)";
        case WALMode::SYNC_EVERY: return "同步刷盘(每次fsync)";
        case WALMode::GROUP_COMMIT: return "Group Commit";
...
TestResult run_test(WALMode mode, int thread_count, int ops_per_thread) {
    ...
    std::cout << "  ✓ P99延迟: " << std::fixed << std::setprecision(2) << result.p99_latency_us << " μs\n";
...
void print_comparison(const std::vector<TestResult>& results) {
    ...
    for (const auto& [threads, group] : grouped) {
        ...
        std::cout << std::left << std::setw(25) << "模式"
                  << std::right << std::setw(15) << "QPS"
                  << std::setw(15) << "P99延迟(μs)"
                  << std::setw(15) << "性能损耗" << "\n";
        std::cout << std::string(80, '-') << "\n";
        ...
        for (const auto& r : group) {
            ...
            std::cout << std::left << std::setw(25) << wal_mode_name(r.mode)
                      << std::right << std::setw(15) << std::fixed << std::setprecision(0) << r.qps
                      << std::setw(15) << std::fixed << std::setprecision(2) << r.p99_latency_us;
    ...
    double group_commit_loss = (best_baseline_qps - best_group_commit_qps) / best_baseline_qps * 100;
    ...
    std::cout << "   ✓ 在保证持久化的前提下，性能损耗控制在 " 
              << std::fixed << std::setprecision(0) << group_commit_loss << "% 以内！\n\n";
    
    std::cout << "3. 同步刷盘性能: " << std::fixed << std::setprecision(0) << worst_sync_qps << " QPS\n";
    std::cout << "   ✓ Group Commit相比同步刷盘提升 " 
              << std::fixed << std::setprecision(0) << speedup << " 倍！\n\n";
    ...
    std::cout << "    - 开启Group Commit后仍有 " << std::fixed << std::setprecision(1)
              << best_group_commit_qps / 1000000.0 << "M QPS，性能损耗仅 " 
              << std::fixed << std::setprecision(0) << group_commit_loss << "%\n";
    std::cout << "    - 而同步刷盘只有 " << std::fixed << std::setprecision(0) 
              << worst_sync_qps / 1000.0 << "K QPS\n";
    std::cout << "    Group Commit相比同步刷盘性能提升了 " 
              << std::fixed << std::setprecision(0) << speedup << " 倍！\"\n\n";
}

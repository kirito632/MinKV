...
private:
  ...
  ConfigManager() { std::cout << "ConfigManager initialized" << std::endl; }
...
int main() {
    ...
    // 使用延迟初始化单例
    auto& monitor = minkv::base::PerformanceMonitor::GetInstance([](auto&
instance) { std::cout << "PerformanceMonitor lazy initialized" << std::endl;
        // 可以在这里进行复杂的初始化逻辑
    });

...
bool SimpleCheckpointManager<K, V>::checkpoint_now() {
  ...
  try {
    ...
    std::cout << "[Checkpoint] Acquiring exclusive lock for atomic export..."
              << std::endl;
...
bool SimpleCheckpointManager<K, V>::should_checkpoint() const {
  ...
  auto elapsed_minutes =
      std::chrono::minutes((current_time - last_checkpoint_time) / (1000 * 60));
  ...
  if (wal_size >= config_.wal_size_threshold) {
    std::cout << "[CheckpointTrigger] WAL size threshold reached: "
              << wal_size / (1024 * 1024)
              << "MB >= " << config_.wal_size_threshold / (1024 * 1024) << "MB"
              << std::endl;
    return true;
  }
...
void SimpleCheckpointManager<K, V>::start_background_checker() {
  ...
  background_thread_ =
      std::thread(&SimpleCheckpointManager::background_checker_loop, this);
...
bool SimpleCheckpointManager<K, V>::recover_from_disk() {
  ...
  try {
    ...
    if (latest_snapshot.empty()) {
      std::cout << "[Recovery] No snapshot found, replaying full WAL from lsn=0"
                << std::endl;
    } else {
      if (!read_snapshot_file(latest_snapshot, snapshot_data, snapshot_lsn)) {
        std::cerr << "[Recovery] Failed to read snapshot: " << latest_snapshot
                  << std::endl;
        return false;
      }
...
bool SimpleCheckpointManager<K, V>::read_snapshot_file(
  ...
  try {
    ...
    std::cout << "[Snapshot] Reading " << header.record_count
              << " records, LSN: " << wal_lsn << std::endl;
...
template <typename K, typename V>
uint32_t SimpleCheckpointManager<K, V>::calculate_checksum(

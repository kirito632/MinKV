...
void GraphStore::AddEdge(const Edge &edge) {
  ...
  // Step 2: 更新出边邻接表（AdjListAdd 内部去重，多条不同 label 的边只记录一次
  // dst）
...
void GraphStore::RebuildAdjacencyList() {
  ...
  // all_data 析构，export_all_data 的 unique_lock 释放
...
std::vector<std::string> GraphStore::FindPath(const std::string &src_id,
                                              const std::string &dst_id,
                                              int max_hops) const {
  // 特殊情况：起点等于终点
  if (src_id == dst_id)
...
GraphStore::SearchSimilarNodes(const std::vector<float> &query_embedding,
  ...
  for (const auto &[k, v] : all_data) {
    ...
    // id。 为保持一致性，我们存储原始 node_id（从 VecKey 逆推不现实），
    // 改为直接从 Edge 反序列化获取。实际上 VecKey = "vec:" +
    // EscapeId(node_id)， 但我们在 SetNodeEmbedding 时没有存原始 id。
    // 解决方案：遍历时用 GetNode 验证，或者直接把 Key 后缀当 node_id 传给
    // GetNode。 由于 EscapeId 是单射的，这里我们存储转义后的 id 作为 key， 但
    // GetNode 接受的是原始 id。 正确做法：在 SearchSimilarNodes 中，node_id
    // 应该是原始 id。 我们通过 export_all_data 拿到的 Key 是
    // "vec:{escaped_id}"， 需要去掉前缀后 unescape 得到原始 id。 简化：由于
    // EscapeId 只转义 ':' 和 '\'，我们实现 UnescapeId。

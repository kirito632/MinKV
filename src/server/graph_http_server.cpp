 ...
 *
 * 接口：
 *   POST /graph/add_node    {"node_id":"...","properties_json":"...","embedding":[...]}
 *   POST /graph/add_edge    {"src_id":"...","dst_id":"...","label":"...","weight":1.0}
 *   POST /graph/rag_query   {"query_embedding":[...],"vector_top_k":3,"hop_depth":2}
 *   GET  /health
 *
 * 编译：
...
static void handle_add_node(const httplib::Request& req, httplib::Response& res) {
    try {
        ...
        if (!body.contains("node_id")) { send_err(res, 400, "missing node_id"); return; }
...
static void handle_add_edge(const httplib::Request& req, httplib::Response& res) {
    try {
        ...
        if (!body.contains("src_id") || !body.contains("dst_id") || !body.contains("label")) {
            send_err(res, 400, "missing src_id/dst_id/label"); return;
        }
        ...
        send_ok(res, {{"success", true}, {"src_id", e.src_id}, {"dst_id", e.dst_id}, {"label", e.label}});
    } catch (const std::exception& e) {
        send_err(res, 500, e.what());
    }
}
...
static void handle_rag_query(const httplib::Request& req, httplib::Response& res) {
    try {
        ...
        for (const auto& n : nodes) {
            nodes_json.push_back({{"node_id", n.node_id}, {"properties_json", n.properties_json}});
        }
...
int main(int argc, char* argv[]) {
    ...
    svr.Get("/health", [](const httplib::Request&, httplib::Response& res) {
        res.set_content(R"({"status":"ok","service":"MinKV Graph HTTP Server"})", "application/json");
    });

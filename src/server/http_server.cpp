...
HttpServer::HttpServer(
    ...
    int port
) : kv_(kv), graph_store_(graph_store), host_(host), port_(port), running_(false) {
...
void HttpServer::setup_routes() {
    ...
    server_->Post("/vector/put", [this](const httplib::Request& req, httplib::Response& res) {
        handle_vector_put(req, res);
    });
    server_->Post("/vector/search", [this](const httplib::Request& req, httplib::Response& res) {
        handle_vector_search(req, res);
    });
    server_->Get("/vector/get", [this](const httplib::Request& req, httplib::Response& res) {
        handle_vector_get(req, res);
    });
    server_->Delete("/vector/delete", [this](const httplib::Request& req, httplib::Response& res) {
    ...
    if (graph_store_) {
        server_->Post("/graph/add_node", [this](const httplib::Request& req, httplib::Response& res) {
            handle_graph_add_node(req, res);
        });
        server_->Post("/graph/add_edge", [this](const httplib::Request& req, httplib::Response& res) {
            handle_graph_add_edge(req, res);
        });
        server_->Post("/graph/rag_query", [this](const httplib::Request& req, httplib::Response& res) {
...
bool HttpServer::start_async() {
    ...
    server_thread_ = std::make_unique<std::thread>([this]() {
        std::cout << "[HttpServer] 启动服务器（异步）：" << host_ << ":" << port_ << std::endl;
...
void HttpServer::handle_vector_search(const httplib::Request& req, httplib::Response& res) {
    try {
        json request_body = json::parse(req.body);
...
void HttpServer::handle_vector_delete(const httplib::Request& req, httplib::Response& res) {
    try {
        if (!req.has_param("key")) {
            send_error(res, 400, "缺少必填查询参数：key");
            return;
...
void HttpServer::send_error(httplib::Response& res, int status_code, const std::string& message) {
...
void HttpServer::handle_graph_add_node(const httplib::Request& req, httplib::Response& res) {
    try {
        json body = json::parse(req.body);
...
void HttpServer::handle_graph_add_edge(const httplib::Request& req, httplib::Response& res) {
    try {
        json body = json::parse(req.body);
...
void HttpServer::handle_graph_rag_query(const httplib::Request& req, httplib::Response& res) {
    try {
        json body = json::parse(req.body);

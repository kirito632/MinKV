 ...
 * // 定期删除服务通过 startExpirationService() 手动启动，或直接使用
 * ShardedCache 的 RAII 方式
 *
...
public:
   ...
   * @return MinKV实例的智能指针
   */
  static std::unique_ptr<MinKV> create(size_t capacity_per_shard = 1000,
   ...
   * @brief 获取数据
   */
  std::optional<V> get(const K &key) { return cache_->get(key); }
   ...
   * @brief 删除数据
   */
  bool remove(const K &key) { return cache_->remove(key); }
   ...
   * @brief 禁用持久化
   */
  void disablePersistence() { cache_->disable_persistence(); }
  ...
  void recoverFromDisk() {
    if (!checkpoint_mgr_) {
      std::cerr << "[MinKV] recoverFromDisk: persistence not enabled, call "
   ...
   * @brief 创建数据快照
   */
  void createSnapshot() { cache_->create_snapshot(); }
   ...
   * @brief 停止定期删除服务
   */
  void stopExpirationService() { cache_->stopExpirationService(); }
   ...
   * @brief 获取向量数据
   */
  std::vector<float> vectorGet(const K &key) { return cache_->vectorGet(key); }
   ...
   * @brief 获取性能统计
   */
  db::CacheStats getStats() const { return cache_->getStats(); }
   ...
   * @brief 执行健康检查
   */
  void performHealthCheck() { cache_->performHealthCheck(); }

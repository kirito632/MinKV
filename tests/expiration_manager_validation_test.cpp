...
void testAutoStart() {
  ...
  auto callback = [&callback_count](size_t shard_id,
    ...
    std::cout << "  [Callback] shard=" << shard_id << ", sample=" << sample_size
              << std::endl;
    return 0;
  };
...
void testParameterValidation() {
  ...
  // 测试无效分片数
  try {
    base::ExpirationManager mgr(valid_callback, 0,
  ...
  // 测试无效检查间隔
  try {
    base::ExpirationManager mgr(valid_callback, 4, std::chrono::milliseconds(0),
                                10);
    assert(false && "应该抛出异常");
  } catch (const std::invalid_argument &e) {
  ...
  // 测试无效采样大小
  try {
    base::ExpirationManager mgr(valid_callback, 4,
...
void testExceptionSafety() {
  ...
  // 创建会抛出异常的回调
  auto throwing_callback = [&callback_count](size_t shard_id,
                                             size_t) -> size_t {
  ...
  {
    base::ExpirationManager mgr(throwing_callback, 4,
                                std::chrono::milliseconds(50), 10);
...
void testStatistics() {
  ...
  auto callback = [&total_expired](size_t shard_id,
                                   size_t sample_size) -> size_t {
    // 模拟删除一些过期key

...
void test_vector_search_integration() {
    ...
    std::cout << "✅ 向量搜索集成测试通过，找到 " << search_results.size() << " 个结果" << std::endl;
...
void test_concurrent_operations() {
    ...
    std::cout << "并发测试完成: " 
              << "总操作=" << (final_stats.puts + final_stats.hits + final_stats.misses)
              << ", 当前大小=" << final_stats.current_size
              << ", 命中率=" << (100.0 * final_stats.hits / (final_stats.hits + final_stats.misses)) << "%" << std::endl;
...
void test_comprehensive_integration() {
    ...
    std::cout << "  - 平均过期比例: " << (expiration_stats.avg_expired_ratio * 100) << "%" << std::endl;
    
    std::cout << "健康状态:" << std::endl;
    std::cout << "  - 整体健康: " << (health_status.overall_healthy ? "是" : "否") << std::endl;
    std::cout << "  - 健康分片: " << health_status.healthy_shards << "/" << health_status.total_shards << std::endl;

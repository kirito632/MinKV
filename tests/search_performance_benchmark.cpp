...
public:
  ...
  static BenchmarkResult run_benchmark(const BenchmarkConfig &config) {
    ...
    double total_time_s =
        duration_cast<milliseconds>(total_end - total_start).count() / 1000.0;
  ...
  static void export_csv(const vector<BenchmarkResult> &results,
    ...
    // CSV头部
    file << "VectorCount,Dimension,K,P50_ms,P95_ms,P99_ms,Avg_ms,Min_ms,Max_ms,"
            "QPS,MeetsRequirement\n";

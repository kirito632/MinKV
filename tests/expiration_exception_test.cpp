...
bool test_callback_std_exception() {
  ...
  TEST_ASSERT(count_after > count_before,
              "Thread should continue running after exceptions");
...
bool test_callback_unknown_exception() {
  ...
  TEST_ASSERT(count_after > count_before,
              "Thread should continue running after unknown exceptions");
...
bool test_mixed_exceptions() {
  ...
  TEST_ASSERT(count_after > count_before,
              "Thread should continue running with mixed exceptions");
...
bool test_all_shards_throw() {
  ...
  // 验证：回调被持续调用（线程没有崩溃）
  TEST_ASSERT(call_count.load() > 16,
              "Thread should continue calling callback despite all failures");
  ...
  TEST_ASSERT(count_after > count_before,
              "Thread should continue running even when all callbacks fail");
...
int main() {
  ...
  // 运行所有测试
  if (test_constructor_validation())
    passed++;
  else
    failed++;
  if (test_callback_std_exception())
    passed++;
  else
    failed++;
  if (test_callback_unknown_exception())
    passed++;
  else
    failed++;
  if (test_mixed_exceptions())
    passed++;
  else
    failed++;
  if (test_all_shards_throw())
    passed++;
  else
    failed++;
  if (test_stats_with_exceptions())
    passed++;
  else
    failed++;

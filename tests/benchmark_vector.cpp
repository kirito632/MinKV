...
int main() {
  ...
  auto duration_ref =
      std::chrono::duration_cast<std::chrono::milliseconds>(end_ref - start_ref)
          .count();
  ...
  for (int q = 0; q < NUM_QUERIES; ++q) {
    for (const auto &vec : database) {
      // 使用我们写的 AVX2 实现
      sum_avx +=
          VectorOps::L2DistanceSquare_AVX2(query.data(), vec.data(), DIM);
    }
  }
  auto end_avx = std::chrono::high_resolution_clock::now();
  auto duration_avx =
      std::chrono::duration_cast<std::chrono::milliseconds>(end_avx - start_avx)
          .count();

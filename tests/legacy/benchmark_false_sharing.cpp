...
private:
  ...
  static void benchmark_cache_performance() {
    ...
    // 计算吞吐量
    double original_qps =
        (double)(NUM_THREADS * OPERATIONS) / original_duration.count() * 1000;
    double optimized_qps =
        (double)(NUM_THREADS * OPERATIONS) / optimized_duration.count() * 1000;

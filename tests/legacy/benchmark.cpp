...
BenchmarkResult benchmark_single_lock() {
  ...
  return {.name = "Single Lock (LruCache)",
          .total_ops = total_ops,
...
BenchmarkResult benchmark_sharded_lock(int shard_count) {
  ShardedCache<std::string, std::string> cache(
      10000 / shard_count, shard_count); // 总容量 10000，分散到多个分片
  ...
  return {.name = "Sharded Lock (" + std::to_string(shard_count) + " shards)",
          .total_ops = total_ops,
          .duration_ms = duration_ms,
          .qps = qps};
}

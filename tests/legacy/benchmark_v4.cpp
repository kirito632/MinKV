...
public:
  ...
  ~AlignedShardedLruCache() { delete[] shards_; }
...
BenchmarkResult benchmark_simple_lru() {
  ...
  return {.name = "SimpleLruCache (单机大锁)",
          .total_ops = total_ops,
...
BenchmarkResult benchmark_sharded_lru() {
  ...
  return {.name = "ShardedLruCache(32)",
          .total_ops = total_ops,
          .duration_ms = duration_ms,
          .qps = qps,
          .latency_us = latency_us};
...
BenchmarkResult benchmark_aligned_sharded_lru() {
  ...
  return {.name = "AlignedShardedLruCache(32)",
          .total_ops = total_ops,
          .duration_ms = duration_ms,
          .qps = qps,
          .latency_us = latency_us};
...
int main() {
  ...
  cout << "  ✓ 分片锁性能提升: " << fixed << setprecision(1)
       << (speedup_sharded - 1) * 100 << "%\n";

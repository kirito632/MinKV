...
private:
  WALBenchmarkResult run_single_test(int thread_count, bool wal_enabled) {
    ...
    for (int i = 0; i < thread_count; ++i) {
      workers.emplace_back([this, &cache]() { this->worker_thread(cache); });
    }
  ...
  void calculate_latency_stats(WALBenchmarkResult &result) {
    ...
    if (p99_idx >= latencies_.size())
      p99_idx = latencies_.size() - 1;
...
void save_results_to_csv(const std::vector<WALBenchmarkResult> &results) {
  ...
  // CSV头部
  csv << "Scenario,WAL_Enabled,Thread_Count,QPS,Avg_Latency_us,P99_Latency_us,"
         "Hit_Rate,Total_Ops,Duration_sec\n";
...
void print_summary_analysis(const std::vector<WALBenchmarkResult> &results) {
  ...
  std::cout << "Threads | WAL OFF QPS | WAL ON QPS  | Performance Drop | "
               "Latency Impact\n";
  std::cout << "--------|-------------|-------------|------------------|-------"
               "---------\n";

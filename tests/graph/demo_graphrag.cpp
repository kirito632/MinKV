...
static std::vector<float> perturb(const std::vector<float>& base, float noise, unsigned seed) {
    std::mt19937 rng(seed);
...
static bool test_graphrag_association() {
    ...
    std::cout << "\n[查询向量] 与 Elon_Musk embedding 高度相似（模拟'钢铁侠的航天梦'）\n";
    ...
    for (auto& n : rag2) {
        if (n.node_id == "Starship") { found_starship = true; break; }
    }
    ...
    std::cout << "  Graph-on-KV：顺着 founded→product 关系链，额外挖掘出 SpaceX 和 Starship\n";
...
static void test_performance() {
    ...
    std::cout << "\n[测试配置] " << N_NODES << " 节点, " << N_EDGES
              << " 边, " << DIM << " 维 embedding, 重复 " << N_RUNS << " 次\n\n";
    std::cout << std::fixed << std::setprecision(1);
    std::cout << "  操作                      avg(μs)   P99(μs)\n";
    std::cout << "  ─────────────────────────────────────────────\n";
    std::cout << "  SearchSimilarNodes         "
              << std::setw(7) << vec_avg  << "   " << std::setw(7) << vec_p99  << "\n";
    std::cout << "  GraphRAGQuery (1-hop)      "
              << std::setw(7) << rag1_avg << "   " << std::setw(7) << rag1_p99 << "\n";
    std::cout << "  GraphRAGQuery (2-hop)      "
              << std::setw(7) << rag2_avg << "   " << std::setw(7) << rag2_p99 << "\n";
...
static bool test_consistency() {
    ...
    for (auto& [k, v] : all_data) {
        if (k.size() > 2 && k.substr(0, 2) == "e:") ++edge_count;
        if (k.size() > 4 && k.substr(0, 4) == "adj:") ++adj_count;
    }

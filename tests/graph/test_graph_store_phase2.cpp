...
bool test_delete_node_cascades_adj() {
    ...
    // bob 的节点数据消失
    CHECK(!gs.GetNode("bob").has_value(),
          "DeleteNode: bob's node data gone");
...
bool test_delete_node_removes_embedding() {
    ...
    CHECK(gs.GetNodeEmbedding("node1").empty(),
          "DeleteNode: embedding removed");
 ...
 *   DeleteEdge 后（无其他边）：dst ∉ GetOutNeighbors(src) 且 src ∉ GetInNeighbors(dst)
 *
 * Validates: Requirements 4.1, 4.2, 4.3
 */
static bool run_property7() {
    ...
    bool p7a = rc::check(
        "Property 7a: AddEdge 后邻接表包含对应条目",
        [](const Edge& e) {
...
static bool run_property8() {
    ...
    bool p8a = rc::check(
        "Property 8a: DeleteNode 后 GetNode 返回 nullopt",
        [](const Node& n) {
            auto kv = std::make_shared<GraphKVStore>(1024, 4);
            GraphStore gs(kv);
            gs.AddNode(n);
    ...
    bool p8b = rc::check(
        "Property 8b: DeleteNode 后 GetNodeEmbedding 返回空",
        [](const Node& n) {
            auto kv = std::make_shared<GraphKVStore>(1024, 4);
            GraphStore gs(kv);
            gs.AddNode(n);
    ...
    bool p8c = rc::check(
        ...
        [](const Node& n, const std::string& neighbor_id) {
            ...
            RC_ASSERT(std::find(neighbor_out.begin(), neighbor_out.end(), n.node_id) == neighbor_out.end());
            RC_ASSERT(std::find(neighbor_in.begin(),  neighbor_in.end(),  n.node_id) == neighbor_in.end());
        }
    );
...
int main() {
    ...
    std::cout << "\n=== Unit Test Results: " << passed << " passed, "
              << failed << " failed ===\n";

...
int main() {
    ...
    tool_graph_add_node(gs, "Elon_Musk",  R"({"role":"CEO","desc":"Tesla & SpaceX founder"})", emb_musk);
    tool_graph_add_node(gs, "SpaceX",     R"({"type":"company","desc":"aerospace manufacturer"})", emb_spacex);
    tool_graph_add_node(gs, "Starship",   R"({"type":"rocket","desc":"fully reusable launch vehicle"})");

    tool_graph_add_edge(gs, "Elon_Musk", "SpaceX",   "founded");
    tool_graph_add_edge(gs, "SpaceX",    "Starship",  "product");

    std::cout << "\n[图谱结构]\n";
    std::cout << "  Elon_Musk(有embedding) --[founded]--> SpaceX(有embedding) --[product]--> Starship(无embedding)\n\n";
    ...
    for (auto& n : result2) {
        if (n.node_id == "Elon_Musk")  found_musk    = true;
        if (n.node_id == "SpaceX")     found_spacex  = true;
        if (n.node_id == "Starship")   found_starship = true;
    }
    ...
    all_pass &= check(found_starship, "2-hop 包含 Starship（2-hop 邻居，无 embedding 也能找到）");

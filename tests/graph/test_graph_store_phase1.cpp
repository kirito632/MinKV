 ...
 *   - Property 5: Node CRUD 往返 — AddNode(n); GetNode(id) == n；未添加的 id 返回 nullopt
 *     Validates: Requirements 2.1, 2.2, 2.3
 *   - Property 6: Edge CRUD 往返 — AddEdge(e); GetEdge(src,dst,label) == e；未添加的边返回 nullopt
...
struct Arbitrary<Edge> {
    static Gen<Edge> arbitrary() {
        auto finite_float = gen::suchThat(
            gen::arbitrary<float>(),
...
static bool run_property5() {
    ...
    bool p5a = rc::check(
        "Property 5a: AddNode(n); GetNode(id) == n",
        [](const Node& n) {
    ...
    bool p5b = rc::check(
        "Property 5b: GetNode(未添加的 id) == nullopt",
...
static bool run_property6() {
    ...
    bool p6a = rc::check(
        "Property 6a: AddEdge(e); GetEdge(src,dst,label) == e",
        [](const Edge& e) {
            auto kv = std::make_shared<GraphKVStore>(1024, 4);
            GraphStore gs(kv);
            gs.AddEdge(e);
    ...
    bool p6b = rc::check(
        "Property 6b: GetEdge(未添加的三元组) == nullopt",

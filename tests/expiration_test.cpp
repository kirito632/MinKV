...
public:
    struct Item {
        ...
        bool isExpired() const {
            if (expire_time_ms == 0) return false;  // 永不过期
...
public:
    ...
    size_t expirationCallback(size_t shard_id, size_t sample_size) {
        if (shard_id >= shards_.size()) return 0;
...
void testNonBlockingBehavior() {
    ...
    // 业务线程回调
    auto callback = [&cache, &expiration_skips](size_t shard_id, size_t sample_size) -> size_t {
...
void benchmarkExpiration() {
    ...
    size_t final_size = cache.size();
    std::cout << "最终缓存大小: " << final_size << " (预期约 " << (KEY_COUNT / 2) << ")" << std::endl;

...
public:
    ...
    void print_report() const {
        ...
        std::cout << "  最大值: " << max_count << " (偏差 " 
                  << std::fixed << std::setprecision(2)
                  << ((max_count - mean) / mean * 100) << "%)\n";
...
void test_different_shard_counts() {
    ...
    for (size_t shard_count : shard_counts) {
        ...
        std::cout << std::setw(12) << shard_count
                  << std::setw(11) << std::fixed << std::setprecision(2) << (cv * 100) << "%"
                  << std::setw(11) << max_deviation << "%"
                  << std::setw(12) << (cv < 0.10 ? "✅ 良好" : "⚠️  一般") << "\n";
    }

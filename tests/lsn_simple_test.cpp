...
TestResult test_lsn_vs_timestamp() {
    ...
    try {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            timestamps.push_back(std::chrono::duration_cast<std::chrono::milliseconds>(duration).count());
        }
        ...
        std::cout << "   LSN duplicates: " << lsn_duplicates << " / " << lsns.size() << std::endl;
        std::cout << "   Timestamp duplicates: " << timestamp_duplicates << " / " << timestamps.size() << std::endl;
        ...
        if (timestamp_duplicates > 0) {
            std::cout << "   ⚠️  Timestamp has duplicates (this is expected in fast operations)" << std::endl;
        }
        
        std::cout << "✅ LSN vs Timestamp comparison test PASSED" << std::endl;
        std::cout << "   LSN provides better uniqueness guarantee than timestamp" << std::endl;
...
TestResult test_current_lsn_no_increment() {
    ...
    try {
        ...
        if (lsn1 != lsn2 || lsn2 != lsn3) {
            result.fail("current_lsn() is incrementing: " + 
                       std::to_string(lsn1) + ", " + 
                       std::to_string(lsn2) + ", " + 
                       std::to_string(lsn3));
            return result;
        ...
        std::cout << "   current_lsn() correctly returns current value without incrementing" << std::endl;
...
TestResult test_lsn_performance() {
    ...
    try {
        ...
        std::cout << "   " << iterations << " LSN allocations: " << duration.count() << " μs" << std::endl;
        std::cout << "   Average: " << avg_time_us << " μs per LSN" << std::endl;
        
        // 性能要求：平均每次LSN分配应该 < 0.1μs
        if (avg_time_us > 0.1) {
            result.fail("LSN allocation too slow: " + std::to_string(avg_time_us) + " μs per LSN");
            return result;
        }
...
int main() {
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        if (results[i].passed) {
            ...
            failed++;
            std::cout << "Test " << (i+1) << ": ❌ FAILED - " << results[i].error_message << std::endl;
        }
    }

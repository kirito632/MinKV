...
struct LatencyStats {
    ...
    void report(const std::string& prefix = "") {
        ...
        std::cout << prefix << "Avg: " << std::fixed << std::setprecision(2) << avg << "μs" << std::endl;
...
void benchmark_basic_rw(DB& db, const std::string& name, int num_threads) {
    std::cout << "\n=== " << name << " - 基础读写性能 (线程数: " << num_threads << ") ===" << std::endl;
    ...
    std::cout << "QPS: " << std::fixed << std::setprecision(0) << qps << " (" << qps / 10000 << "万)" << std::endl;
}
...
void benchmark_scalability(DB& db, const std::string& name) {
    ...
    std::cout << std::left << std::setw(8) << "线程数" 
              << std::setw(15) << "QPS" 
              << std::setw(15) << "QPS(万)"
              << std::setw(12) << "扩展效率" << std::endl;
    ...
    for (int num_threads : thread_counts) {
        ...
        std::cout << std::left << std::setw(8) << num_threads
                  ...
                  << std::setw(15) << std::setprecision(1) << (qps / 10000)
...
void benchmark_latency(DB& db, const std::string& name, int num_threads) {
    std::cout << "\n=== " << name << " - 延迟分布测试 (线程数: " << num_threads << ") ===" << std::endl;
...
void generate_comparison_report() {
    ...
    std::cout << "\n" << std::left 
              << std::setw(12) << "线程数"
              << std::setw(18) << "StdMap QPS(万)"
              << std::setw(18) << "MinKV QPS(万)"
              << std::setw(15) << "提升倍数"
